...
function showLoading(msg) {
    ...
    if (loaderText) loaderText.innerText = msg || 'ระบบกำลังประมวลผล กรุณารอสักครู่';
    if (!loader) return;
...
function requireArrayResult(res, fallbackMessage = 'รูปแบบข้อมูลจากระบบไม่ถูกต้อง') {
    throwIfApiFailure(res, fallbackMessage);
    if (!Array.isArray(res)) throw new Error(fallbackMessage);
    return res;
}
...
async function applyStudentMenuSettings() {
    try {
        ...
        throwIfApiFailure(permissions, 'ไม่สามารถดึงสิทธิ์เมนูนักศึกษาได้');
        if (!permissions || !permissions.globalSettings) throw new Error('ไม่พบข้อมูลการตั้งค่าเมนูจากระบบ');
...
async function updateUserDashboard() {
    ...
    if (currentUser.profileImage && currentUser.profileImage !== 'undefined' && currentUser.profileImage !== "") {
        ...
        imgEl.src = "https://drive.google.com/thumbnail?id=" + imageId + "&sz=w400";
...
async function loadUserProfile() {
    ...
    try {
        ...
        throwIfApiFailure(profile, 'ไม่สามารถดึงข้อมูลทะเบียนประวัติได้');
        hideLoading();
        if (profile) {
        ...
        hideLoading();
        console.error(error);
        showAlert(error.message || 'ไม่สามารถดึงข้อมูลทะเบียนประวัติได้', 'error');
    }
}
...
async function loadActivitiesForUser() {
    showLoading();
    try {
        const data = requireArrayResult(await callApi('getActivities', { 
            mode: 'user', 
            studentId: currentUser.studentId, 
            token: userToken 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // V3: preload ข้อมูลประจำหน้า "ทุกเมนูนักศึกษา" หลัง Login
    const studentBasePayload = {
        studentId: currentUser.studentId,
        token: userToken
    };

    const studentPreloadTasks = [
        { action: 'getStudentMenuPermissions', payload: studentBasePayload },
        { action: 'checkStudentEligibility2569', payload: studentBasePayload },
        { action: 'getProfile', payload: studentBasePayload },
        { action: 'getActivities', payload: { mode: 'user', ...studentBasePayload } },
        { action: 'getStudentActivityHistory', payload: studentBasePayload },
        { action: 'getMyQueue', payload: studentBasePayload },
        { action: 'getQueueSlots', payload: { role: 'user', ...studentBasePayload } },
        { action: 'checkStudentOverEligibility', payload: studentBasePayload },
        { action: 'checkStudentResignStatus', payload: studentBasePayload },
        { action: 'getMyPetitions', payload: studentBasePayload },
        { action: 'checkTransferStatus', payload: { action: 'checkTransferStatus', ...studentBasePayload } }
    ];

    scheduleApiPreload(studentPreloadTasks, {
        startDelayMs: 300,
        concurrency: 2,
        gapMs: 90
    });

    window.refreshAllStudentMenuCache = async () => {
        clearApiCache();
        return apiPreloadQueue(studentPreloadTasks, { concurrency: 2, gapMs: 90 });
    };
        ...
        reader.onload = async function(e) {
            ...
            try {
                ...
                if (res.success) {
                    ...
                    imgEl.src = "https://drive.google.com/thumbnail?id=" + safeImageId + "&sz=w400";
                    imgEl.referrerPolicy = "no-referrer";
                    imgEl.style.display = 'block';
