...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // โหลดข้อมูลหน้าที่นักศึกษาใช้บ่อยไว้ล่วงหน้าแบบไม่บล็อกหน้าหลัก
    setTimeout(() => apiPrefetch('getProfile', {
        studentId: currentUser.studentId,
        token: userToken
    }), 350);
    setTimeout(() => apiPrefetch('getActivities', {
        mode: 'user',
        studentId: currentUser.studentId,
        token: userToken
    }), 850);
    setTimeout(() => apiPrefetch('getStudentActivityHistory', {
        studentId: currentUser.studentId,
        token: userToken
    }), 1250);
