...
document.addEventListener("DOMContentLoaded", () => {
        ...
        try {
            ...
            if (res.success) {
                ...
                if (res.user.mustChangePassword === true) {
                    const changeResult = await Swal.fire({
                        icon: 'warning', title: 'ต้องตั้งรหัสผ่านใหม่',
                        html: `<div style="text-align:left;font-size:14px;margin-bottom:10px;">รหัสผ่านที่ใช้เข้าสู่ระบบเป็นรหัสชั่วคราว กรุณาตั้งรหัสผ่านใหม่ก่อนเข้าสู่หน้าหลัก</div>
                          <input id="swalNewPassword" type="password" class="swal2-input" placeholder="รหัสผ่านใหม่อย่างน้อย 8 ตัวอักษร" autocomplete="new-password">
                          <input id="swalConfirmPassword" type="password" class="swal2-input" placeholder="ยืนยันรหัสผ่านใหม่" autocomplete="new-password">`,
                        confirmButtonText: 'บันทึกรหัสผ่านใหม่', confirmButtonColor: 'var(--secondary-color)',
                        ...
                        preConfirm: async () => {
                            ...
                            if(newPassword.length<8){Swal.showValidationMessage('รหัสผ่านต้องมีอย่างน้อย 8 ตัวอักษร');return false;}
                            if(newPassword!==confirmPassword){Swal.showValidationMessage('รหัสผ่านยืนยันไม่ตรงกัน');return false;}
                            try { const changed=await callApi('changeTemporaryPassword',{studentId:res.user.studentId,token:res.user.token,newPassword});
                                if(!changed||!changed.success){Swal.showValidationMessage((changed&&changed.message)||'ไม่สามารถเปลี่ยนรหัสผ่านได้');return false;} return changed;
                            } catch(err){Swal.showValidationMessage(err.message||'ไม่สามารถเปลี่ยนรหัสผ่านได้');return false;}
                        }
                    });
                    if(!changeResult.isConfirmed){sessionStorage.clear();return;}
                    res.user.mustChangePassword=false; sessionStorage.setItem('ubu_user_data',JSON.stringify(res.user));
                }

                Swal.fire({ icon:'success', title:'เข้าสู่ระบบสำเร็จ', text:'คลิกปุ่มด้านล่างเพื่อเข้าสู่ระบบ', showConfirmButton:true,
                    confirmButtonText:'เข้าสู่หน้าหลัก <i class="material-icons" style="vertical-align: middle; font-size: 16px;">arrow_forward</i>',
                    confirmButtonColor:'var(--secondary-color)', allowOutsideClick:false
                }).then((result)=>{if(result.isConfirmed)window.location.href=res.user.role==='admin'?'View_Admin.html':'View_User.html';});
            } else {
                if (res.isSuspended) {
                ...
                } else if (res.isTemporaryPasswordExpired) {
                    Swal.fire({ icon:'warning', title:'รหัสผ่านชั่วคราวหมดอายุ', text:res.message || 'กรุณาติดต่อเจ้าหน้าที่เพื่อขอรหัสผ่านชั่วคราวใหม่', confirmButtonText:'ตกลง', confirmButtonColor:'#f57c00' });
    ...
    document.getElementById('resetPasswordForm').addEventListener('submit', async (e) => {
        ...
        if (newPass.length < 8) { showAlert('รหัสผ่านต้องมีความยาวอย่างน้อย 8 ตัวอักษร', 'warning'); return; }
